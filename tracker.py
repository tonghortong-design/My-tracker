    ...
    if not df.empty:
        ...
        if not expense_df.empty:
            ...
            with tab1:
                ...
                # 使用 Streamlit 內建圓餅圖
                st.pie_chart(data=cate_chart, values="金額", names="類別", use_container_width=True)
                
            with tab2:
                st.write("#### 每日花費走勢")
                # 按日期分組加總，並補齊時間軸
                df_trend = expense_df.groupby("日期")["金額"].sum().reset_index()
                df_trend = df_trend.set_index("日期")
                st.line_chart(df_trend["金額"], use_container_width=True)
        else:
